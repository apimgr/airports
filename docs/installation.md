...
chown -R 1000:1000 volumes/config volumes/data
docker run --rm -d \
  --name airports \
  -p 8080:80 \
...
The container runs as a fixed non-root UID/GID (`1000:1000`); pre-chowning the
volume directories avoids a permission-denied failure on first start.
