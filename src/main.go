...
func run(port string, devMode bool) error {
	...
	if connStr != "" {
		...
		dbPath := getEnv("DB_PATH", fmt.Sprintf("%s/airports.db", dataDir))
	...
	// Save credentials to file if this is first initialization
	if creds.Token != "" {
		if err := database.SaveCredentialsToFile(creds, configDir); err != nil {
...
func printHelp() {
	...
	fmt.Println("                      sqlite:/data/airports.db")
	fmt.Println("                      mysql://user:pass@localhost:3306/dbname")
	fmt.Println("                      postgres://user:pass@localhost:5432/dbname")
	fmt.Println("  DB_TYPE           Database type (sqlite, mysql, postgres)")
	fmt.Println("  DB_PATH           SQLite database file path")
	fmt.Println()
	...
	fmt.Println("  Web UI:  http://localhost:8080/admin")
	fmt.Println("  API:     http://localhost:8080/api/v1/admin")
	fmt.Println()
